...
class IssueReturnController extends Controller
{
    ...
    public function getAssetForReturn(Request $request)
    {
        ...
        if (!empty($requestbenchId)) {
            $clauses .= " and i.bench_id = '$requestbenchId'";
        }
    ...
    public function get(Request $request)
    {
        try {
            ...
            foreach ($issue as $key => $item) {
                if ($item->bench_id != null || $item->bench_id != '') {
                    $item->table = DB::select("select * from benches where id = ?", [$item->bench_id])[0];
                } else {
                    $item->table = null;
                }
            }
    ...
    public function details(Request $request) 
    {
        try {
            $whereCluase = "";
            if (!empty($requestbenchId)) {
                $whereCluase .= " AND r.id = '$requestbenchId'";
            }
            ...
            $details = DB::select("SELECT
                        ...
                        LEFT JOIN benches r ON r.id = ir.bench_id
                        WHERE ird.status != 'd' 
                        $whereCluase");
