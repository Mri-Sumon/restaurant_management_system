...
class BookingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($bookings as $key => $booking) {
            $booking->booking_details = DB::select("select bkd.*,
                                                        ...
                                                        left join benches rm on rm.id = bkd.bench
                                                        left join bench_types tp on tp.id = rm.bench_type_id
    ...
    public function getTableList(Request $request)
    {
        ...
        foreach ($floors as $key => $floor) {
            ...
            foreach ($floor->benches as  $item) {
                $tablebooked = DB::select("select * from benches rm where rm.id in (select ot.benche from order_tables ot where ot.status = 'a' and ot.booking_status = 'booked') and rm.id = ?", [$item->id]);
    ...
    public function singleAvailableTable(Request $request)
    {
        ...
        for ($i = 0; $i < $totalDays; $i++) {
            ...
            $check = DB::select("select * from benches rm where rm.id not in (select bkd.benche from booking_details bkd where bkd.status = 'a' and bkd.booking_status != 'checkout' and bkd.checkout_status = 'true' $clauses and '$date' between bkd.checkin_date and bkd.checkout_date) and rm.id = ?", [$request->id]);
    ...
    public function tableBookingCalendar(Request $request)
    {
        ...
        from booking_details bkd
        where bkd.bench_id = ?
        and bkd.status = 'a' and bkd.booking_status != 'checkout'", [$requestbenchId]);
    ...
    public function getCheckinList(Request $request)
    {
        ...
        if (!empty($request->typeId)) {
            $clauses .= " and r.bench_type_id = '$request->typeId'";
        }
        ...
        if (!empty($requestbenchId)) {
            $clauses .= " and r.id = '$requestbenchId'";
        }

        $checkin_date = date('Y-m-d') . ' 12:00:00';

        $benches = DB::select("select r.*,
                                rt.name as tabletype_name
                                from benches r
                                left join bench_types rt on rt.id = r.bench_type_id
                                where r.status != 'd' and r.deleted_at is null
                                $clauses
                                ");

        foreach ($benches as $key => $item) {
            $tableavailable = DB::select("select * from benches rm where rm.id not in (select bkd.bench_id from booking_details bkd where bkd.status = 'a' and '$checkin_date' between bkd.checkin_date and bkd.checkout_date) and rm.id = ?", [$item->id]);
            $item->available = count($tableavailable) > 0 ? 'true' : 'false';
            $tablebooked = DB::select("select * from benches rm where rm.id in (select bkd.bench_id from booking_details bkd where bkd.status = 'a' and bkd.booking_status = 'booked' and '$checkin_date' between bkd.checkin_date and bkd.checkout_date) and rm.id = ?", [$item->id]);
            $item->booked = count($tablebooked) > 0 ? 'true' : 'false';
            $tablecheckin = DB::select("select * from benches rm where rm.id in (select bkd.bench_id from booking_details bkd where bkd.status = 'a' and bkd.booking_status = 'checkin' and '$checkin_date' between bkd.checkin_date and bkd.checkout_date) and rm.id = ?", [$item->id]);
            ...
            if ($item->available == 'true') {
                ...
                $item->color = '#ff0000ab';
                $item->details = DB::select("select * from booking_details bkd where bkd.status = 'a' and '$checkin_date' between bkd.checkin_date and bkd.checkout_date and bkd.bench_id = ? and bkd.booking_status = 'checkin'", [$item->id]);
        ...
        $benches = array_filter($benches, function ($item) {
            return $item->checkin == 'true';
        });

        return response()->json(array_values($benches), 200);
    }
