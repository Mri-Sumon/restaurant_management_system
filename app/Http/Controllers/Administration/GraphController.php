    ...
    public function index()
    {
        try {
            ...
            $totalBookedTable = DB::select("
                    ...
                    from benches t
                    where t.status = 'a'
                    and t.id in (select ot.bench_id from orders ot where ot.status = 'p')
            ...
            $totalAvailableTable = DB::select("
                    ...
                    from benches t
                    where t.status = 'a'
                    and t.id not in (select ot.bench_id from orders ot where ot.status = 'p')
            ...
            $checkIn = count(array_values(array_filter($benches, function ($item) {
                return $item->checkin == 'true';
            })));

            $vacant = count(array_values(array_filter($benches, function ($item) {
