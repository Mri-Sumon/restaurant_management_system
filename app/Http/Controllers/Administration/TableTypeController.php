...
class TableTypeController extends Controller
{
    ...
    {
        $tableTypes = BenchType::where('status', 'a')->latest()->get();
    ...
    public function store(Request $request)
    {
        ...
        $tabletypename = BenchType::where('name', $request->name)->first();
        if (!empty($tabletypename)) return send_error("This name have been already exists", null, 422);
        try {
            $check = DB::table('bench_types')->where('deleted_at', '!=', NULL)->where('name', $request->name)->first();;
            if (!empty($check)) {
                DB::select("UPDATE bench_types SET deleted_by = NULL, deleted_at = NULL , status = 'a' WHERE id = ?", [$check->id]);
            } else {
                $data = new BenchType();
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) return send_error("Validation Error", $validator->errors(), 422);
        $tabletypename = BenchType::where('id', '!=', $request->id)->where('name', $request->name)->first();
        if (!empty($tabletypename)) return send_error("This name have been already exists", null, 422);
        try {
            $data = BenchType::find($request->id);
            $data->name = $request->name;
            $data->slug = make_slug($request->name);
