...
class ServiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($requestbenchId)) {
            $clauses .= " AND s.bench_id = '$requestbenchId'";
        }
        ...
        $services = DB::select("SELECT s.*,
                            ...
                            FROM services s
                            LEFT JOIN benches r ON r.id = s.bench_id
    ...
    {
        $detail = BookingDetail::where('bench_id', $requestbenchId)->where('checkin_date', '<=', $request->date . ' 12:00:00')->where('checkout_date', '>=', $request->date . ' 11:59:00')->where('booking_status', 'checkin')->orderBy('id', 'DESC')->first();
        if ($detail) {
