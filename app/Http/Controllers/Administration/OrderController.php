...
class OrderController extends Controller
{
    ...
    public function store(OrderRequest $request)
    {
        ...
        try {
            ...
            $benchId = $request->tableCart[0]['bench_id'];
            $chairId = gettype($request->tableCart[0]['chairName']) == "string" ? $request->tableCart[0]['chair_id'] : $request->tableCart[0]['chairName']['id'];

            $order->bench_id = $benchId;
            ...
            foreach ($request->carts as $cart) {
                ...
                if (isset($recipes)) {
                    ...
                    $production->total = $recipes->sum('total') * $cart['quantity'];
                    ...
                    foreach ($recipes as $item) {
                        ...
                        $productionDetails->quantity = $item->quantity * $cart['quantity'];
    ...
    public function updateDraft(OrderRequest $request)
    {
        ...
        try {
            ...
            // dd(gettype($request->tableCart[0]['chairName']) == "string" ? $request->tableCart[0]['chair_id'] : $request->tableCart[0]['chairName']['id']);
            // dd(gettype($request->tableCart[0]['chairName']));
            // dd($request->tableCart[0]['chair_id']);
    ...
    public function getOrder(Request $request)
    {
        try {
            ...
            if (!empty($requestbenchId)) {
                array_push($whereCluase, ['bench_id', '=', $requestbenchId]);
            }
            ...
            if (!empty($request->kitchen_status)) {
                array_push($whereCluase, ['kitchen_status', '=', $request->kitchen_status]);
            }
            ...
            if ((!empty($request->recordType) && $request->recordType == 'with') || !empty($request->id)) {
                $orders = Order::with('orderDetails', 'customer', 'table', 'chair', 'bank', 'user')->where($whereCluase)->latest('id');
            } else {
                $orders = Order::with('customer', 'table', 'chair', 'bank', 'user')->where($whereCluase)->latest('id');
            }
    ...
    public function orderDetailsByChair(Request $request)
    {
        try {
            $orders = DB::select("SELECT id FROM orders WHERE status = 'p' AND bench_id = '$requestbenchId' AND chair_id = '$request->chairId' ORDER BY date DESC LIMIT 1");
            return response()->json($orders);
        } catch (\Throwable $th) {
            return send_error("Something went wrong", $th->getMessage());
        }
    ...
    public function destroy(Request $request)
    {
        try {
            ...
            $productions = Production::where('order_id', $request->orderID)->get();

            if (isset($productions) && count($productions) > 0) {
                foreach ($productions as $pro) {
                    // delete production details----------
                    $pDetail = ProductionDetail::where('production_id', $pro->id)->get();
                    if (isset($pDetail) && count($pDetail) > 0) {
                        foreach ($pDetail as $pdata) {
                            ProductionDetail::find($pdata->id)->forceDelete();
                        }
                    }
                    // delete production ----------
                    Production::find($pro->id)->forceDelete();
                }
            }

            $olddetail = OrderDetails::where('order_id', $request->orderID)->get();
            if (isset($olddetail) && count($olddetail) > 0) {
                foreach ($olddetail as $item) {
                    OrderDetails::find($item->id)->forceDelete();
                }
            }
    ...
    {
        if (!checkAccess('pending_delivery')) {
            return view('error.unauthorize');
        }
        return view('administration.restaurant.kitchenPendingOrders');
    }

    public function kitchenApprove(Request $request)
    {
        try {
            DB::beginTransaction();
            $data = Order::find($request->id);
            $data->kitchen_status = 'a';
            $data->added_by = Auth::user()->id;
            $data->last_update_ip = request()->ip();
            $data->update();
    ...
    public function deliveredOrderList()
    {
        ...
        return view('administration.restaurant.kitchenDeliveredOrders');
    }
}
