    ...
    <form @submit.prevent="updateAbout">
        <div class="row" style="margin:0;">
            <div class="col-md-2 aboutpageImage">
                ...
                <fieldset class="scheduler-border bg-of-skyblue" style="margin-top: 10px;">
                    <legend class="scheduler-border">Home BG Image</legend>
                    <div class="control-group">
                        <div class="form-group ImageBackground clearfix">
                            <span class="text-danger">(1920 X 1080)PX</span>
                            <img :src="homeBgImageSrc" class="imageShow" />
                            <label for="home_bg_image">Upload</label>
                            <input type="file" id="home_bg_image" class="form-control shadow-none" @change="homeBgImageUrl" />
                        </div>
                    </div>
                </fieldset>
            </div>
    ...
    new Vue({
        ...
        methods: {
            getAbout() {
                axios.get("/get-about")
                    .then(res => {
                        ...
                        this.homeBgImageSrc = r.home_bg_image ? '/'+r.home_bg_image : '/noImage.gif';
            ...
            updateAbout(event) {
                ...
                formdata.append('home_bg_image', this.aboutpage.home_bg_image);
            ...
            imageUrl(event) {
                this.handleImageUpload(event, 'imageSrc', 'image', 600, 400);
            },

            homeBgImageUrl(event) {
                this.handleImageUpload(event, 'homeBgImageSrc', 'home_bg_image', 1920, 1080);
            },

            handleImageUpload(event, srcField, dataField, width, height) {
                if (event.target.files[0]) {
                    ...
                    reader.onload = (ev) => {
                        ...
                        img.onload = async e => {
                            ...
                            this[srcField] = new_img_url;
                            const resizedImage = await new Promise(rs => canvas.toBlob(rs, 'image/jpeg', 1))
                            this.aboutpage[dataField] = new File([resizedImage], event.target.files[0].name, {
