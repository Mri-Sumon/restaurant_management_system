@extends('master')
@section('title', 'Disposal List')
@section('breadcrumb_title', 'Disposal List')
@push('style')
<style scoped>
...
<div id="disposalList">
    <div class="row" style="margin:0;">
        <fieldset class="scheduler-border bg-of-skyblue">
            <legend class="scheduler-border">Search Disposal List</legend>
            <div class="control-group">
                <form @submit.prevent="getDisposal">
    ...
    <div class="row" style="display: flex;justify-content:space-between;">
        <div class="col-md-3" v-if="disposals2.length > 0" style="display:none;" :style="{display: disposals2.length > 0 ? '' : 'none'}">
            <input type="search" @input="filterArray($event)" placeholder="Search..." class="form-control">
        </div>
        <div class="col-md-9 text-right">
            <a v-if="disposals.length > 0" style="display:none;" :style="{display: disposals.length > 0 ? '' : 'none'}" href="" style="margin: 7px 0;display:block;width:50px;" v-on:click.prevent="print">
                <i class="fa fa-print"></i> Print
            </a>
        </div>
    </div>
    <div class="row" v-if="disposals.length > 0 && showReport" style="display:none;" :style="{display: disposals.length > 0 && showReport ? '' : 'none'}">
        <div class="col-md-12">
            <div class="table-responsive" id="reportTable">
                <table class="table table-bordered record-table table-condensed" v-if="filter.searchType != 'quantity' && filter.recordType == 'with'" style="display:none;" :style="{display: filter.searchType != 'quantity' && filter.recordType == 'with'? '': 'none'}">
                    ...
                    <tbody>
                        <template v-for="(disposal, index) in disposals">
                            <tr>
                                <td>@{{ index + 1 }}</td>
                                <td>@{{ disposal.invoice }}</td>
                                <td>@{{ disposal.date | dateFormat("DD-MM-YYYY") }}</td>
                                <td>@{{disposal.table.name }}</td>
                                <td>@{{ disposal.disposal_details[0].asset.name }}</td>
                                <td style="text-align:center;">@{{ disposal.disposal_details[0].price | decimal }}</td>
                                <td style="text-align:center;">@{{ disposal.disposal_details[0].quantity | decimal }}</td>
                                <td style="text-align:center;">@{{ disposal.disposal_details[0].disposal_status }}</td>
                                <td style="text-align:right;">@{{ disposal.disposal_details[0].total | decimal }}</td>
                                <td style="text-align:center;">
                                    @if(userAction('u'))
                                    <a href="" title="Edit Order" @click.prevent="disposalEdit(disposal)"><i class="fa fa-edit"></i></a>
                                    @endif
                                    @if(userAction('d'))
                                    <a href="" title="Delete Order" @click.prevent="deleteDisposal(disposal)"><i class="fa fa-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            <tr v-for="(item, sl) in disposal.disposal_details.slice(1)">
                                <td colspan="4" :rowspan="disposal.disposal_details.length - 1" v-if="sl == 0"></td>
                                <td>@{{ item.asset.name }}</td>
                                <td style="text-align:center;">@{{ item.price | decimal }}</td>
                                <td style="text-align:center;">@{{ item.quantity | decimal }}</td>
                                <td style="text-align:center;">@{{ item.disposal_status }}</td>
                                <td style="text-align:right;">@{{ item.total | decimal }}</td>
                                <td></td>
                            </tr>
                            <tr style="font-weight:bold;">
                                <td colspan="6" style="font-weight:normal;text-align:left;"><strong>Note: </strong>@{{ disposal.note }}</td>
                                <td style="text-align:center;">Total Quantity<br>@{{ disposal.disposal_details.reduce((prev, curr) => {return prev + parseFloat(curr.quantity)}, 0) | decimal }}</td>
                                <td></td>
                                <td style="text-align:right;">
                                    Total: @{{ disposal.total | decimal }}
                                </td>
                                <td></td>
                            </tr>
                        </template>
                ...
                <table class="table table-bordered record-table table-condensed" v-if="filter.searchType != 'quantity' && filter.recordType == 'without'" style="display:none;" :style="{display: filter.searchType != 'quantity' && filter.recordType == 'without'? '': 'none'}">
                    ...
                    <tbody>
                        <tr v-for="(disposal, index) in disposals">
                            <td>@{{ index + 1 }}</td>
                            <td>@{{ disposal.invoice }}</td>
                            <td>@{{ disposal.date | dateFormat("DD-MM-YYYY") }}</td>
                            <td>@{{ disposal.table.name }}</td>
                            <td style="text-align:right;">@{{ disposal.total | decimal }}</td>
                            <td style="text-align:left;">@{{ disposal.note }}</td>
                            <td style="text-align:center;">
                                @if(userAction('u'))
                                <a href="" title="Edit Order" @click.prevent="disposalEdit(disposal)"><i class="fa fa-edit"></i></a>
                                @endif
                        ...
                        <tr style="font-weight:bold;">
                            ...
                            <td style="text-align:right;">@{{ disposals.reduce((prev, curr)=>{return prev + parseFloat(curr.total)}, 0) | decimal }}</td>
                            <td></td>
                            <td></td>
                        </tr>
                    </tbody>
                ...
                <table class="table table-bordered record-table table-condensed" v-if="filter.searchType == 'quantity'" style="display:none;" :style="{display: filter.searchType == 'quantity'? '': 'none'}">
                    ...
                    <tbody>
                        <tr v-for="(disposal, index) in disposals">
                            ...
                            <td>@{{ disposal.invoice }}</td>
                            <td>@{{ disposal.date | dateFormat("DD-MM-YYYY") }}</td>
                            <td>@{{ disposal.table_name }}</td>
                            <td>@{{ disposal.name }}</td>
                            <td style="text-align:right;">@{{ disposal.price }}</td>
                            <td style="text-align:center;">@{{ disposal.disposal_status }}</td>
                            <td style="text-align:right;">@{{ disposal.quantity }}</td>
                        </tr>
                        <tr style="font-weight:bold;">
                            <td colspan="7" style="text-align:right;">Total Quantity</td>
                            <td style="text-align:right;">@{{ disposals.reduce((prev, curr) => { return prev + parseFloat(curr.quantity)}, 0) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
<script>
    new Vue({
        ...
        methods: {
            async disposalEdit(row) {
                location.href = "/disposal/" + row.id
            },
            ...
            getDisposal() {
                if (this.filter.searchType == 'table') {
                    this.filter.benchId = this.selectedTable != null ? this.selectedTable.id : ""
                }
                if (this.filter.searchType == 'quantity') {
                    var url = '/get-disposal-details';
                    this.filter.assetId = this.selectedAsset != null ? this.selectedAsset.id : ""
                } else {
                    var url = "/get-disposal";
                ...
                axios.post(url, this.filter)
                    .then(res => {
                        ...
                        this.disposals2 = this.disposals
            ...
            async deleteDisposal(row) {
                let formdata = {
                    id: row.id
                }
                if (confirm("Are you sure !!")) {
                    axios.post("/delete-disposal", formdata)
                        .then(res => {
            ...
            async print() {
                ...
                let reportContent = `
					<div class="container">
                        <div class="row">
                            <div class="col-xs-12">
                                <h4 style="text-align:center">Disposal Record</h4>
                            </div>
                        </div>
                        <div class="row">
							<div class="col-xs-6">
            ...
            // filter disposalrecord
            filterArray(event) {
                this.disposals = this.disposals2.filter(disposal => {
                    return disposal.invoice.toLowerCase().startsWith(event.target.value.toLowerCase()) ||
                        disposal.date.toLowerCase().startsWith(event.target.value.toLowerCase());
                })
            },
        },
    })
