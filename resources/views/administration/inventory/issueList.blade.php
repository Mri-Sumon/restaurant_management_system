...
@section('breadcrumb_title', 'Issue List')
@push('style')
<style scoped>
...
<div id="issueList">
    <div class="row" style="margin:0;">
        <fieldset class="scheduler-border bg-of-skyblue">
            <legend class="scheduler-border">Search Issue List</legend>
                    ...
                    <tbody>
                        <template v-for="(issue, index) in issues">
                            <tr>
                                ...
                                <td>@{{ issue.issue_details[0].asset.name }}</td>
                                <td style="text-align:center;">@{{ issue.issue_details[0].price | decimal }}</td>
                                <td style="text-align:center;">@{{ issue.issue_details[0].quantity | decimal }}</td>
                                <td style="text-align:right;">@{{ issue.issue_details[0].total | decimal }}</td>
                                <td style="text-align:center;">
                                    <a href="" title="Order Invoice" v-bind:href="`/issue-invoice-print/${issue.id}`" target="_blank"><i class="fa fa-file-text"></i></a>
                                    @if(userAction('u'))
                                    <a href="" title="Edit Order" @click.prevent="issueEdit(issue)"><i class="fa fa-edit"></i></a>
                                    @endif
                                    @if(userAction('d'))
                                    <a href="" title="Delete Order" @click.prevent="deleteIssue(issue)"><i class="fa fa-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            <tr v-for="(item, sl) in issue.issue_details.slice(1)">
                                <td colspan="4" :rowspan="issue.issue_details.length - 1" v-if="sl == 0"></td>
                            ...
                            <tr style="font-weight:bold;">
                                <td colspan="6" style="font-weight:normal;text-align:left;"><strong>Note: </strong>@{{ issue.note }}</td>
                                <td style="text-align:center;">Total Quantity<br>@{{ issue.issue_details.reduce((prev, curr) => {return prev + parseFloat(curr.quantity)}, 0) | decimal }}</td>
                                <td style="text-align:right;">
                                    Total: @{{ issue.total | decimal }}<br>
                                    Vat: @{{ issue.vatAmount | decimal }}
                                </td>
                                <td></td>
                            </tr>
                        </template>
                ...
                <table class="table table-bordered record-table table-condensed" v-if="filter.searchType != 'quantity' && filter.recordType == 'without'" style="display:none;" :style="{display: filter.searchType != 'quantity' && filter.recordType == 'without'? '': 'none'}">
                    ...
                    <tbody>
                        <tr v-for="(issue, index) in issues">
                            ...
                            <td>@{{ issue.date | dateFormat("DD-MM-YYYY") }}</td>
                            <td>@{{ issue.table.name }}</td>
                            <td style="text-align:right;">@{{ issue.sub_total | decimal }}</td>
                            <td style="text-align:right;">@{{ issue.vat | decimal }}</td>
                        ...
                        <tr style="font-weight:bold;">
                            ...
                            <td style="text-align:right;">@{{ issues.reduce((prev, curr)=>{return prev + parseFloat(curr.subtotal)}, 0) | decimal }}</td>
                            <td style="text-align:right;">@{{ issues.reduce((prev, curr)=>{return prev + parseFloat(curr.vat)}, 0) | decimal }}</td>
        ...
        methods: {
            ...
            async deleteIssue(row) {
                ...
                if (confirm("Are you sure !!")) {
                    axios.post("/delete-issue", formdata)
                        .then(res => {
            ...
            async print() {
                ...
                let reportContent = `
					<div class="container">
                        <div class="row">
                            <div class="col-xs-12">
                                <h4 style="text-align:center">issues Record</h4>
                            </div>
                        </div>
                        <div class="row">
							<div class="col-xs-6">
