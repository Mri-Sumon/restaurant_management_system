@extends('master')
@section('title', 'Kitchen Delivered Orders')
@section('breadcrumb_title','Kitchen Delivered Orders')
@push('style')
<style scoped>
                    ...
                    <tbody>
                        <template v-for="(order, index) in orders">
                            <tr>
                                ...
                                <td>@{{ order.table.name }} - @{{ order.chair.name }}</td>
                                ...
                                <td style="text-align:center;">
                                    ...
                                    @endif
                                    {{-- @if(userAction('e'))
                                        <a href="" title="Approve Order" v-if="kitchen_status == 'p'" @click.prevent="approveOrder(order)"><i class="fa fa-check"></i></a>
                                    @endif --}}
                                    {{-- @if(userAction('d'))
                            ...
                            <tr v-for="(item, sl) in order.order_details.slice(1)">
                                <td colspan="5" :rowspan="order.order_details.length - 1" v-if="sl == 0"></td>
                            ...
                            <tr style="font-weight:bold;">
                                <td colspan="7" style="font-weight:normal;text-align:left;"><strong>Note: </strong>@{{ order.note }}</td>
                ...
                <table class="table table-bordered record-table table-condensed" v-if="filter.searchType !='category' && filter.searchType != 'quantity' && filter.recordType == 'without'" style="display:none;" :style="{display: filter.searchType !='category' && filter.searchType != 'quantity' && filter.recordType == 'without'? '': 'none'}">
                    ...
                    <tbody>
                        <tr v-for="(order, index) in orders">
                            ...
                            <td style="text-align:center;">
                                <span v-if="kitchen_status == 'p'" style="color: orange">Pending</span>
                                <span v-else style="color: green">Delivered</span>
                            </td>
                            <td style="text-align:center;">
                                <a href="" title="Order Invoice" v-bind:href="`/order-invoice-print/${order.id}`" target="_blank"><i class="fa fa-file-text"></i></a>
                                @if(userAction('u'))
        ...
        methods: {
            ...
            async approveOrder(row) {
                ...
                if (confirm("Are you sure!")) {
                    axios.post("/kitchen_approve_order", formdata).then(res => {
