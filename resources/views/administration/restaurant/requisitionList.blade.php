...
@section('breadcrumb_title', 'Requisition List')
@push('style')
    <style scoped>
...
<div id="requisitionList">
    <div class="row" style="margin:0;">
        <fieldset class="scheduler-border bg-of-skyblue">
            <legend class="scheduler-border">Search Requisition List</legend>
            <div class="control-group">
                <form @submit.prevent="getPurchase">
                    ...
                    <div class="col-md-2 col-xs-12">
                        {{-- v-if="filter.searchType == 'status'" style="display: none;" :style="{display: filter.searchType == 'status' ? '' : 'none'}"> --}}
                        <div class="form-group" style="display: flex;align-items:center;">
                            <label for="" style="width:150px;">Status</label>
                            <select class="form-select no-padding" style="width: 100%;" v-model="filter.status">
                                <option value="">All</option>
                    ...
                    <tbody>
                        <template v-for="(purchase, index) in purchases">
                            <tr>
                                ...
                                <td>@{{ purchase.requisition_details[0].material.name }}</td>
                                <td style="text-align:center;">@{{ purchase.requisition_details[0].price | decimal }}</td>
                                <td style="text-align:center;">@{{ purchase.requisition_details[0].quantity | decimal }}</td>
                                <td style="text-align:right;">@{{ purchase.requisition_details[0].total | decimal }}</td>
                                <td>
                                    <span v-if="purchase.manager_status == 'p'" style="color: red">
                                        Pending
                                    </span>
                                ...
                                <td>
                                    <span v-if="purchase.admin_status == 'p'" style="color: red">
                                        Pending
                                    </span>
                                    <span v-else style="color: green">
                                        Active
                                ...
                                <td style="text-align:center;">
                                    <a href="" title="Requisition Invoice" v-bind:href="`/requisition-invoice-print/${purchase.id}`" target="_blank"><i class="fa fa-file-text"></i></a>
                                    @if(userAction('u'))
                                        <a href="" title="Edit Requisition" @click.prevent="requisitionEdit(purchase)"><i class="fa fa-edit"></i></a>
                                    @endif
                                    @if(userAction('manager'))
                                        <a href="" v-if="purchase.manager_status == 'p' || purchase.admin_status == 'p'" title="Approve Requisition" @click.prevent="requisitionStatus(purchase.id)"><i class="fa fa-check"></i></a>
                                    @endif
                                    @if(userAction('d'))
                                        <a href="" title="Delete Requisition" @click.prevent="deleteRequisition(purchase)"><i class="fa fa-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            <tr v-for="(item, sl) in purchase.requisition_details.slice(1)">
                                <td colspan="4" :rowspan="purchase.requisition_details.length - 1" v-if="sl == 0"></td>
                            ...
                            <tr style="font-weight:bold;">
                                ...
                                <td style="text-align:center;">Total Quantity<br>@{{ purchase.requisition_details.reduce((prev, curr) => {return prev + parseFloat(curr.quantity)}, 0) | decimal }}</td>
...
<script>
    new Vue({
        ...
        methods: {
            async requisitionEdit(row) {
                location.href = "/requisition/" + row.id
            },
            ...
            async deleteRequisition(row) {
                let formdata = {
                    id: row.id
                }
                if (confirm("Are you sure !!")) {
                    axios.post("/delete-requisition", formdata)
                    .then(res => {
            ...
            requisitionStatus(requisitionId){
                if (confirm("Are you sure !!")) {
                    axios.post("/requisition_status", {id: requisitionId})
                    .then(res => {
                        toastr.success(res.data.message)
                        this.getPurchase();
                    })
            ...
            async print() {
                ...
                let reportContent = `
					<div class="container">
                        <div class="row">
                            <div class="col-xs-12">
                                <h4 style="text-align:center">purchases Record</h4>
                            </div>
                        </div>
                        <div class="row">
							<div class="col-xs-6">
